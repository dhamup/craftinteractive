erpnext.PointOfSale.Controller = class {
	...
	prepare_additem_btn() {
		this.page.add_button(__("Add Item"), null, { btn_class: "btn-info additem-btn" });
		this.bind_additem_events();
	}

	bind_additem_events() {
		this.$additem = this.page.page_actions.find(".additem-btn");
					...
					frappe.call({
						...
						args: {
							doc: {
								...
								opening_stock: values.stock_qty,
								standard_rate: values.rate,
								description: values.description,
								brand:values.brand,
								taxes: [
									{
										item_tax_template: values.item_tax,
									}
								],
							}
						},
						callback: function(response) {
							if (response.message) {
								frappe.msgprint("Item Created: " + response.message.name);
							}
						}
					});
                    d.hide();
