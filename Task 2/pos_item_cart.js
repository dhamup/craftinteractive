erpnext.PointOfSale.ItemCart = class {
				...
				onchange: function () {
					if (this.value) {
						...
						frm.script_manager.trigger("customer", frm.doc.doctype, frm.doc.name).then(() => {
							frappe.run_serially([
								() => me.fetch_customer_last_purchase(this.value),
	...
	fetch_customer_last_purchase(customer) {
		frappe.call({
			method: "demo_app.custom_code.pos_invoice_details",
			args: { customer: customer },
			callback: function(response) {
				if (response.message) {
					let invoices = response.message;
					let d = new frappe.ui.Dialog({
						title: 'Customer Last Purchase Details',
						size: "extra-large",
						fields: [
							{
					...
					invoices.forEach(invoice => {
						invoice.items.forEach(item => {
							table_field.df.data = table_field.df.data || [];
